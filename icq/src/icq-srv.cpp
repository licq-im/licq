    ...
    unsigned short groupId)
{
  ...
  {
    CPU_AddToServerList* pAdd = new CPU_AddToServerList(groupName);
  ...
  CPU_AddToServerList* pAdd = new CPU_AddToServerList(userId, ICQ_ROSTxNORMAL,
...
void IcqProtocol::icqChangeGroup(const Licq::UserId& userId)
{
  ...
  if (oldGSID != 0)
  {
    ...
    CSrvPacketTcp* pRemove = new CPU_RemoveFromServerList(userId, oldGSID, nSID, ICQ_ROSTxNORMAL);
    addToModifyUsers(pRemove->SubSequence(), userId.accountId());
    SendExpectEvent_Server(pRemove, NULL);
  }

  // Add the user, with the new group
  CPU_AddToServerList* pAdd = new CPU_AddToServerList(userId, ICQ_ROSTxNORMAL, newGroup);
  addToModifyUsers(pAdd->SubSequence(), userId.accountId());
  SendExpectEvent_Server(pAdd, NULL);
}
...
void IcqProtocol::icqRemoveUser(const Licq::UserId& userId, bool ignored)
{
  ...
  if (UseServerContactList())
  {
    ...
    CSrvPacketTcp *pRemove = new CPU_RemoveFromServerList(userId,
    ...
    if (nVisibleSID)
    {
      CSrvPacketTcp* pVisRemove = new CPU_RemoveFromServerList(userId, 0,
    ...
    if (nInvisibleSID)
    {
      CSrvPacketTcp* pInvisRemove = new CPU_RemoveFromServerList(userId, 0,
...
void IcqProtocol::icqRemoveGroup(const Licq::ProtoRemoveGroupSignal* ps)
{
  ...
  CSrvPacketTcp* pRemove = new CPU_RemoveFromServerList(ps->groupName(), ps->groupServerId());
...
void IcqProtocol::icqRenameGroup(const Licq::ProtoRenameGroupSignal* ps)
{
  ...
  CSrvPacketTcp* pUpdate = new CPU_UpdateToServerList(newName, serverId);
...
void IcqProtocol::icqRenameUser(const Licq::UserId& userId)
{
  ...
  CSrvPacketTcp* pUpdate = new CPU_UpdateToServerList(userId, ICQ_ROSTxNORMAL);
...
void IcqProtocol::icqRequestMetaInfo(const Licq::UserId& userId, const Licq::ProtocolSignal* ps)
{
  ...
  if (bIsAIM)
    ...
    p = new CPU_Meta_RequestAllInfo(userId);
...
void IcqProtocol::icqAddToVisibleList(const Licq::UserId& userId)
{
  ...
  if (UseServerContactList())
  {
    CSrvPacketTcp* pAdd = new CPU_AddToServerList(userId, ICQ_ROSTxVISIBLE);
    addToModifyUsers(pAdd->SubSequence(), userId.accountId());
    SendExpectEvent_Server(pAdd, NULL);
  }
}
...
void IcqProtocol::icqRemoveFromVisibleList(const Licq::UserId& userId)
{
  ...
  if (UseServerContactList())
  {
    ...
    if (u.isLocked())
    {
      CSrvPacketTcp* pRemove = new CPU_RemoveFromServerList(userId, 0, u->GetVisibleSID(), ICQ_ROSTxVISIBLE);
      addToModifyUsers(pRemove->SubSequence(), userId.accountId());
...
void IcqProtocol::icqAddToInvisibleList(const Licq::UserId& userId)
{
  ...
  if (UseServerContactList())
  {
    CSrvPacketTcp* pAdd = new CPU_AddToServerList(userId, ICQ_ROSTxINVISIBLE);
    addToModifyUsers(pAdd->SubSequence(), userId.accountId());
...
void IcqProtocol::icqRemoveFromInvisibleList(const Licq::UserId& userId)
{
  ...
  if (UseServerContactList())
  {
    ...
    if (u.isLocked())
    {
      CSrvPacketTcp* pRemove = new CPU_RemoveFromServerList(userId, 0, u->GetInvisibleSID(), ICQ_ROSTxINVISIBLE);
      addToModifyUsers(pRemove->SubSequence(), userId.accountId());
...
void IcqProtocol::icqAddToIgnoreList(const Licq::UserId& userId)
{
  ...
  CPU_AddToServerList* pAdd = new CPU_AddToServerList(userId, ICQ_ROSTxIGNORE, 0, false);
  SendEvent_Server(pAdd);
}
...
//      userIP = BSWAP_32(userIP);
      UserWriteGuard u(Licq::UserId(myOwnerId, id));
      if (!u.isLocked())
      {
  ...
  case ICQ_SNACxSUB_OFFLINExLIST:
  {
    ...
    {
        Licq::UserReadGuard user(Licq::UserId(myOwnerId, id));
      //XXX Debug output
  ...
  case ICQ_SNACxMSG_ICBMxERROR:
  {
    ...
    else
    {
                                             ...
                                             ++iter)
      {
        ...
        {
            {
              Licq::UserReadGuard u(Licq::UserId(myOwnerId, (*iter)->myIdString));
              if (!u.isLocked())
...
*/
  ...
  case ICQ_SNACxMSG_TYPING:
  {
    ...
    unsigned short nTyping = packet.UnpackUnsignedShortBE();
      ...
      Licq::UserWriteGuard u(Licq::UserId(myOwnerId, id));
      if (!u.isLocked())
      {
      ...
      do
      {
        ...
        switch (e->SubType())
        {
          ...
          case ICQ_SNACxLIST_ROSTxREM:
          {
            ...
            {
              if (pending.length())
                icqAddUserServer(Licq::UserId(myOwnerId, pending), true);
              break;
            }
            ...
            if (bHandled == false)
            {
              ...
              if (e->ExtraInfo() == 0)
              {
                ...
                if (nListTime == 0)
                  pReply = new CPU_AddToServerList("", false, true);
                else
                  pReply = new CPU_UpdateToServerList("", 0);
                gLog.info(tr("Updating top level group."));
              }
              ...
              {
                pReply = new CPU_UpdateToServerList(groupName, e->ExtraInfo());
            ...
            {
              Licq::UserWriteGuard u(Licq::UserId(myOwnerId, pending));
              if (u.isLocked())
              {
          ...
          case ICQ_SNACxLIST_ROSTxUPD_GROUP:
          {
            if (bHandled == false)
            {
              ...
              if (nError == 0x0E)
              {
                pReply = new CPU_UpdateToServerList(Licq::UserId(myOwnerId, pending), ICQ_ROSTxNORMAL, true);
    ...
    case 0x07D0: // XXX Now I see this one, huh?
    {
      ...
      else if (nSubtype == ICQ_CMDxMETA_RANDOMxUSERxRSP)
      {
        ...
        if (e != NULL && nResult == META_SUCCESS)
        {
          ...
          snprintf(szUin, sizeof(szUin), "%lu", nUin);
              Licq::UserId userId(myOwnerId, szUin);
               ...
               nSubtype == 0x01a4 || nSubtype == 0x01ae)
      {
        ...
        snprintf(foundAccount, sizeof(foundAccount), "%lu", nFoundUin);
            Licq::UserId foundUserId(myOwnerId, foundAccount);
...
void IcqProtocol::ProcessUserList()
{
  ...
  {
    Licq::UserId userId(myOwnerId, iter->first);
    CUserProperties* data = iter->second;
