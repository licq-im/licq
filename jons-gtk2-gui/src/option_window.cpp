...
/*********************** FIRST TAB *********************/ 
	...
	gtk_widget_set_size_request(ow->txtTimestampFormat, 80, 20);
...
/**************** Second tab: Contact List Colors ****************/
	...
	*chg_on_color = 1;
	...
	g_signal_connect(G_OBJECT(online_browse), "clicked",
				  G_CALLBACK(show_on_color_dlg),
	...
	*chg_off_color = 2;
	...
	g_signal_connect(G_OBJECT(offline_browse), "clicked",
			   G_CALLBACK(show_on_color_dlg),
	...
	*chg_away_color = 3;
	...
	g_signal_connect(G_OBJECT(away_browse), "clicked",
		G_CALLBACK(show_on_color_dlg), (gpointer)chg_away_color);
...
/********************* Third tab: Network *************************/
	...
	ow->lstServers = gtk_tree_view_new_with_model(GTK_TREE_MODEL(store));
	g_object_unref(G_OBJECT(store));
	GtkCellRenderer *r_text = gtk_cell_renderer_text_new();
	GtkTreeViewColumn *col = 
			gtk_tree_view_column_new_with_attributes(
				"Server", r_text, "text", 0, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(ow->lstServers), col);
	col = gtk_tree_view_column_new_with_attributes(
				"Port", r_text,	"text", 0, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(ow->lstServers), col);
	...
	gtk_widget_set_size_request(ow->txtFirewallHost, 100, 15);
	gtk_box_pack_start(GTK_BOX(boxHBox), ow->txtFirewallHost, false, false, 1);
...
/********************* END OF ALL THE TABS ************************/
	...
	g_signal_connect(G_OBJECT(close), "clicked",
			   G_CALLBACK(done_options), (gpointer)ow);
...
void show_on_color_dlg(GtkWidget *widget, gpointer data)
{
	...
	g_object_set_data(G_OBJECT(color_dialog), "user", (gpointer)change);
	...
	g_signal_connect(
		G_OBJECT(GTK_COLOR_SELECTION_DIALOG(color_dialog)->ok_button),
		"clicked", G_CALLBACK(color_dlg_ok), (gpointer)color_dialog);
	g_signal_connect(
		G_OBJECT(GTK_COLOR_SELECTION_DIALOG(color_dialog)->cancel_button),
		"clicked", G_CALLBACK(color_dlg_cancel),
...
void color_dlg_ok(GtkWidget *widget, gpointer data)
{
	...
	gint *change = (gint *)g_object_get_data(G_OBJECT(color_dialog), "user");
