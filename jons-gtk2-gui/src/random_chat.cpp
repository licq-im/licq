...
void random_chat_search_window()
{
	...
	rcw->search = gtk_button_new_with_mnemonic("_Search");
	...
	g_signal_connect(G_OBJECT(rcw->search), "clicked",
		G_CALLBACK(random_search_callback), 0);
	g_signal_connect(G_OBJECT(cancel), "clicked",
		G_CALLBACK(random_cancel_callback), 0);
	g_signal_connect(G_OBJECT(close), "clicked",
		G_CALLBACK(random_close_callback), 0);
...
void set_random_chat_window()
{
	...
	src->search = gtk_button_new_with_mnemonic("_Set");
	src->close = gtk_button_new_from_stock(GTK_STOCK_CLOSE);
	GtkWidget *h_box = gtk_hbox_new(true, 0);
	...
	g_signal_connect(G_OBJECT(src->search), "clicked",
		G_CALLBACK(set_random_set_callback), (gpointer)src);
	g_signal_connect(G_OBJECT(src->close), "clicked",
		G_CALLBACK(set_random_close_callback), (gpointer)src);
