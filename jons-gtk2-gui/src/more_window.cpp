...
void list_more_window(GtkWidget *widget, ICQUser *u)
{
	...
	mw->custom_text = gtk_text_view_new();
	gtk_text_view_set_editable(GTK_TEXT_VIEW(mw->custom_text), TRUE);
	GtkTextBuffer *tb = gtk_text_view_get_buffer(GTK_TEXT_VIEW(mw->custom_text));
	gtk_text_buffer_set_text(tb, mw->user->CustomAutoResponse(), -1);
